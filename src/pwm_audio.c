 ...
 */
static inline void IRAM_ATTR ledc_set_left_duty_fast(uint32_t duty_val)
{
    *g_ledc_left_duty_val = (duty_val) << 4; /* Discard decimal part */
    *g_ledc_left_conf0_val |= 0x00000014;
    *g_ledc_left_conf1_val |= 0x80000000;
}
static inline void IRAM_ATTR ledc_set_right_duty_fast(uint32_t duty_val)
{
...
esp_err_t pwm_audio_init(const pwm_audio_config_t *cfg)
{
    ...
    PWM_AUDIO_CHECK(cfg->duty_resolution <= 11 && cfg->duty_resolution >= 8, PWM_AUDIO_RESOLUTION_ERROR, ESP_ERR_INVALID_ARG);
